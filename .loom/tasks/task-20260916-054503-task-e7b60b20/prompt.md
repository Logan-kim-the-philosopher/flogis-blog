...
- Job ID: `job-20260916-054456-2026-09-16-2e03d991`
- Title: 2026-09-16 소크 방향성 회의 전사
- Goal: 바탕화면의 26-09-16_소크_방향성_회의.m4a 전체를 한국어로 전사하고 원본과 같은 경로에 읽기 쉬운 텍스트 전사본을 저장한다.
- Status: `PENDING`
...
- Task ID: `task-20260916-054503-task-e7b60b20`
- Title: 소크 방향성 회의 전사본 저장
- Description: 지정한 m4a 전체를 설치된 로컬 한국어 모델로 구간별 전사하고, 발화 순서와 의미를 보존하면서 명확한 문장부호·띄어쓰기 및 오인식만 보정한다. 불확실한 이름·용어·숫자는 표시하고 원본과 같은 기본 이름의 TXT를 같은 디렉터리에 제공한다.
- Expected output: 바탕화면의 26-09-16_소크_방향성_회의.txt 전사 파일
- Done condition: 입력의 실제 길이 전체를 구간 전사하고 비어 있지 않은 한국어 본문을 원본과 같은 디렉터리에 저장한다. 구간 커버리지·출력 바이트 일치·원본 불변을 확인하고 Loom 검증과 로컬 기록 커밋을 완료한다.
- Validation hint: ## 검증 구획에 ffprobe 입력 길이, 구간 합계, 출력 섹션·본문·바이트 일치 및 원본 SHA-256 불변과 loom validate 통과를 기록한다.
- Required docs: -
- Memory refs: -
- Document outputs: -
...
- In scope: 지정 음성 읽기, 설치된 OpenSuperWhisper 한국어 모델 사용, 임시 오디오 변환과 구간별 대조, 문맥 검수, 같은 경로 TXT 저장, Loom 기록·검증·로컬 커밋
